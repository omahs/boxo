...
func (i *handler) serveDirectory(ctx context.Context, w http.ResponseWriter, r *http.Request, resolvedPath path.ImmutablePath, rq *requestData, isHeadRequest bool, directoryMetadata *directoryMetadata, ranges []ByteRange) bool {
	...
	if err == nil {
		...
		// write to request
		success := i.serveFile(ctx, w, r, resolvedPath, rq, idxFile, "text/html")
		if success {
			i.unixfsDirIndexGetMetric.WithLabelValues(originalContentPath.Namespace().String()).Observe(time.Since(rq.begin).Seconds())
		}
		return success
	}
	...
	// Update metrics
	i.unixfsGenDirListingGetMetric.WithLabelValues(rq.contentPath.Namespace().String()).Observe(time.Since(rq.begin).Seconds())
	return true
}
